...
public class GetVaccinedInfoService implements ClientUtil {
    ...
    //selenium使用的driver
    private static final String SYSTEM_DRIVER;
    //chromedirver系统路径
    private static final String SYSTEM_PATH ;
    //累计接踵人次截圖FileName
    private static final String cumuFileName;
    //各梯次疫苗涵蓋率图FileName
    private static final String coverFileName;
     ...
     * @param body
     */
    public void crawlVaccinedAmount(String body,StringBuilder fullUrl) throws IOException {
        ...
        if(Objects.nonNull(dateNum) && CheckPDFRecordServiceImpl.ISNEWPDF.equals(isNew)){
            ...
            if(Objects.isNull(pdfRecord)){
                log.info("pdf解析成功、pdf内文新增成功  pdf记录至db失败");
            }
            //儲存至static resource
            title += ".pdf";
